...
def check_drone_battery_task():
    drones_below_25 = Drone.objects.filter(battery_capacity__lt=25).count()
    drones_between_25_and_75 = Drone.objects.filter(battery_capacity__range=[25, 75]).count()
    drones_more_than_75 = Drone.objects.filter(battery_capacity__gt=75).count()
